...
const CardBoxModal = ({ title, buttonColor, buttonLabel, isActive, children, onConfirm, onCancel }: Props) => {
  ...
  return (
    ...
    <OverlayLayer onClick={onCancel || (() => console.log('click'))} className={onCancel ? 'cursor-pointer' : ''}>
      <CardBox
      ...
      >
        <CardBoxComponentTitle title={title}>
          {!!onCancel && <BaseButton icon={mdiClose} color="whiteDark" onClick={onCancel} small roundedFull />}
