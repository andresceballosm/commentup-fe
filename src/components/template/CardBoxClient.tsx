    ...
    <CardBox className="mb-6 last:mb-0">
      <div className="flex flex-col md:flex-row items-center justify-between">
        <div className="flex flex-col md:flex-row items-center justify-start mb-6 md:mb-0">
          <UserAvatar className="w-12 h-12 md:mr-6 mb-6 md:mb-0" username={client.name} />
          <div className="text-center md:text-left overflow-hidden">
            <h4 className="text-xl text-ellipsis">{client.name}</h4>
            <p className="text-gray-500 dark:text-slate-400">
              {client.created} @ {client.login}
            </p>
          </div>
        </div>

        <PillTag color={pillColor()} icon={pillIcon} label={`${client.progress}%`} />
      </div>
    </CardBox>
  );
};
